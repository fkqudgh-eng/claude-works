...
// @version      4.3
// @description  주간메뉴표 xlsx → 보드명 자동 파싱 → 미리보기 후 메뉴보드 자동기입. 사이트 프로파일·양식검증·self-test 내장. Save 버튼은 절대 누르지 않음.
...
(function () {
  ...
  function applyPlan(plan) {
    ...
    let done = 0;
    rows.forEach(r => {
      if (!r.지원) return; // 미지원(샐러드)은 이름/설명 보존
      const base = r.idx * 6;
    ...
    if (plusbar != null && plusbar !== '') {
      for (let i = 0; i < slotCount; i++) { setVal(menu[i * 6 + 5], plusbar); pb++; }
    }
    updateUndoBtn();
    ...
    toast(`${plan.meal} ${plan.day}요일 · [${slotTxt}] ${done}칸${pbTxt} (Save 미클릭)${skipTxt}${over}`);
  }
  ...
  async function showPreview() {
    ...
    const pbRow = (plan.plusbar != null && plan.plusbar !== '')
      ? `<tr style="border-top:2px solid #00838f;color:#00838f;background:#f7fdfe">
           ...
           <td style="padding:4px 6px" colspan="2"><b>${esc(plan.plusbar)}</b> <small>(모든 슬롯 동일)</small></td></tr>`
      : '';
    ...
    m.innerHTML = `
      ...
      <table style="width:100%;border-collapse:collapse">
        <tr style="background:#f4f4f4"><th style="padding:4px 6px;text-align:left">코너</th><th style="padding:4px 6px;text-align:left">신규(→기입)</th><th style="padding:4px 6px;text-align:left">기존</th></tr>
        ${rowHtml}${pbRow}</table>
    ...
    m.querySelector('#__maf_apply').onclick = () => { applyPlan(plan); m.remove(); };
  }
